...
ME['connect/listen options parsing'] = function(done) {
  ...
  var opts = [ {server:[port]        , client:[port++]}
             ...
             , {server:[port,{key:key,cert:cert, rejectUnauthorized: false}] 
               ,client:[port,{key:key,cert:cert, rejectUnauthorized: false}]}
             ]

  var p = common.plan(opts.length*2,preDone)
  ...
  function preDone() {
    if (fs.existsSync(unixpath)) return fs.unlink(unixpath,done)
    done()
  }
}
